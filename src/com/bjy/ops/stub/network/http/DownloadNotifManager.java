...
//            if (id != -1) {
//                if (action.equals(LauncherConstant.ACTION_DOWNLOAD_PROGRESS)) {
...
//                                    total, current);
//                }
//                if (action.equals(LauncherConstant.ACTION_DOWNLOAD_COMPOLETED)) {
    ...
    public void addNotification(final long aId,
        ...
        if (aId >= 0) {
            ...
            String[] projection = new String[] {
                    LauncherConstant.COLUMN_DOWNLOAD_TITLE, LauncherConstant.COLUMN_DOWNLOAD_MIME_TYPE
            };

            String selection = LauncherConstant.ID + "=" + aId;

            Cursor cursor = mContext.getContentResolver().query(LauncherConstant.DOWNLOAD_URI, projection,
                    selection, null, "_id DESC");

            String title = "", mime_type = "";

            if (cursor != null && cursor.getCount() > 0 && cursor.moveToFirst()) {
                title = cursor.getString(cursor.getColumnIndex(LauncherConstant.COLUMN_DOWNLOAD_TITLE));
                mime_type = cursor.getString(cursor
                        .getColumnIndex(LauncherConstant.COLUMN_DOWNLOAD_MIME_TYPE));
            }
...
//                    aProgressIntent.putExtra(LauncherConstant.EXTRA_TITLE, title);
//                    aProgressIntent.putExtra(LauncherConstant.EXTRA_ID, aId);
//                    aProgressIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
    ...
    public void updateProgressNotification(long aId, long total, long current) {
        ...
        if (ni != null) {
            ...
            ni.mIntent.putExtra(LauncherConstant.EXTRA_PROGRESS, progress);
...
//            notif.contentIntent =  contentIntent;
    ...
    public void updateCompletedNotification(long aId, int aResult) {
        ...
        if(aResult == LauncherConstant.RESULT_FAILED_SDCARD 
                || aResult == LauncherConstant.RESULT_FAILED_NO_NETWORK
                || aResult == LauncherConstant.RESULT_FAILED_SDCARD_INSUFFICIENT){
            return;
        }

        HashMap<Long, NotificationItem> notifMap = null;
        if (aResult == LauncherConstant.RESULT_SUCCESS) {
            notifMap = mSuccessNotifMap;
        } else if (aResult == LauncherConstant.RESULT_FAILED) {
