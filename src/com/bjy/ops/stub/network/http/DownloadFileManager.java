...
public class DownloadFileManager {
    ...
    public long insertTask(Context context, String url, String dst, String mimetype,
        ...
        if (id == -1) {
            ...
            String selection = LauncherConstant.COLUMN_DOWNLOAD_URL + "=? and " +
                    LauncherConstant.COLUMN_DOWNLOAD_DESTINATION + "=? and " +
                    LauncherConstant.COLUMN_DOWNLOAD_MIME_TYPE + "=?";

            String[] selectionArgs = new String[] {
                    url, dst, mimetype
            };

            Cursor cursor = context.getContentResolver().query(LauncherConstant.DOWNLOAD_URI, projection,
                    selection, selectionArgs, "_id DESC");

            if (cursor != null && cursor.moveToNext()) {
                id = cursor.getLong(cursor.getColumnIndex(LauncherConstant.ID));
                total = cursor.getLong(cursor.getColumnIndex(LauncherConstant.COLUMN_DOWNLOAD_TOTAL_SIZE));
        ...
        if (id == -1) {
            ...
            values.put(LauncherConstant.COLUMN_DOWNLOAD_DESTINATION, dst);
            values.put(LauncherConstant.COLUMN_DOWNLOAD_URL, url);

            values.put(LauncherConstant.COLUMN_DOWNLOAD_MIME_TYPE, mimetype);
            values.put(LauncherConstant.COLUMN_DOWNLOAD_STATUS, HttpTask.State.Idle.ordinal());
            values.put(LauncherConstant.COLUMN_DOWNLOAD_DATE, (new Date()).getTime() / 1000);
            values.put(LauncherConstant.COLUMN_DOWNLOAD_TITLE, title);
            values.put(LauncherConstant.COLUMN_DOWNLOAD_DESCRIPTION, description);
            values.put(LauncherConstant.COLUMN_DOWNLOAD_WIFIONLY, wifiOnly);

            Uri uri = context.getContentResolver().insert(LauncherConstant.DOWNLOAD_URI, values);
        ...
        if (id >= 0) {
            for (DownloadFileTask t : taskList) {
                ...
                if (t.id == id) {
                    ...
                    values.put(LauncherConstant.COLUMN_DOWNLOAD_TOTAL_SIZE, t.mTotalSize);

                    context.getContentResolver().update(LauncherConstant.DOWNLOAD_URI, values,
                            LauncherConstant.ID + "=" + t.id, null);
